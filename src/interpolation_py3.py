...
def to_funct(intervals, polynoms, raiseValueError):
    ''' if the value is out of the intervals and raiseValueError=True,
    ...
    def funct(x):
        if raiseValueError and (x<x_min or x>x_max):
            raise ValueError('function defined between {} and {} (not in {})'.format(x_min, x_max, x))
        elif x<x_min:
...
def interpolate_1d(x, y, d, testing=False, raiseValueError=False):
    ''' Givet two lists x and y s.t. f(x)=y,
...
def test_1d(n,d_max):
    ...
    for i,y in Y_details:
        plt.plot(X_details, y, label=str(i))
...
def test_2d(n,d_max):
    ...
    precision = 10
    X_details = np.linspace(0,1, num=n*precision)
    Y = np.random.normal(size=(*X.shape,dims))
    for i in range(1,n):
        Y[i] += Y[i-1]
